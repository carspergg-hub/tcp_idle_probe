...
async def probe_task(host, port, idle_time, tolerance, delay, result_queue):
    ...
    if delay > 0:
        try: await asyncio.sleep(delay)
            ...
            await result_queue.put((idle_time, "ABORTED")); return
    try:
        ...
        try:
            ...
            status = "ABORTED"; raise
    except asyncio.TimeoutError: status = "TIMEOUT"
    except ConnectionResetError: status = "RESET"
    except Exception as e:
        if not isinstance(e, asyncio.CancelledError): status = "ERROR"
    finally:
        if status != "ABORTED": await result_queue.put((idle_time, status))
...
async def monitor_task(result_queue, tasks, max_fails):
    ...
    while True:
        ...
        if item is None: result_queue.task_done(); break
    ...
    print("\n"+"="*50+"\n 📊 探 测 结 果 统 计 \n"+"="*50)
    for k, v in stats.items(): print(f"  {k:8}: {v}")
    print("="*50)

async def run_client(host, port, start, step, count, tolerance, max_fails, stagger, output):
    if output: sys.stdout = Tee(sys.stdout, open(output, 'a', encoding='utf-8'))
    log(f"[*] 策略: {start}s起步, 步进{step}s, 共{count}次 | 阻断: {max_fails}次")
    result_queue = asyncio.Queue(maxsize=count)
    tasks = [asyncio.create_task(probe_task(host, port, start+i*step, tolerance, i*stagger, result_queue)) for i in range(count)]
    watcher = asyncio.create_task(monitor_task(result_queue, tasks, max_fails))
    await asyncio.gather(*tasks, return_exceptions=True)
    await result_queue.put(None); await watcher

if __name__ == "__main__":
    p = argparse.ArgumentParser()
    sub = p.add_subparsers(dest="role")
    s = sub.add_parser("server")
    s.add_argument("--host", default="0.0.0.0"); s.add_argument("--port", type=int, default=9999); s.add_argument("--tolerance", type=float, default=30.0)
    c = sub.add_parser("client")
    c.add_argument("--host", required=True); c.add_argument("--port", type=int, default=9999); c.add_argument("--start", type=int, default=60); c.add_argument("--step", type=int, default=60); c.add_argument("--count", type=int, default=20); c.add_argument("--tolerance", type=float, default=30.0); c.add_argument("--max-fails", type=int, default=2); c.add_argument("--stagger", type=float, default=0.5); c.add_argument("--output")
    args = p.parse_args()
    if args.role == "server": asyncio.run(run_server(args.host, args.port, args.tolerance))
    elif args.role == "client": asyncio.run(run_client(args.host, args.port, args.start, args.step, args.count, args.tolerance, args.max_fails, args.stagger, args.output))
